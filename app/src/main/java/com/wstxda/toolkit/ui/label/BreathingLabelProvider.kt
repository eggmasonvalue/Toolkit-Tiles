...
class BreathingLabelProvider(private val context: Context) {
    ...
    fun getLabel(phase: BreathingPhase): CharSequence {
        return when (phase) {
            BreathingPhase.IDLE -> context.getString(R.string.breathing_tile)
    ...
    fun getSubtitle(phase: BreathingPhase, seconds: Int): CharSequence {
        return if (phase == BreathingPhase.IDLE) {
            context.getString(R.string.tile_start)
        } else {
            context.getString(R.string.breathing_seconds_format, seconds)
        }
    }
}